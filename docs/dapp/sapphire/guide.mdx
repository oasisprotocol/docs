...
The figure below illustrates the flow of a confidential smart contract
*transaction* executed on the Sapphire ParaTime. Nodes that are running
trusted enclaves are marked with 🔒.

![Confidential Smart Contract Transaction Flow on Sapphire](../diagrams/c10l-smart-contract-tx.mmd.svg)
...
The next figure illustrates the flow of a confidential smart contract *call* on
Sapphire. `msg.sender` will be zeroed for calls that are not signed. If you
want to use `msg.sender` for access control, the call **must be signed**. On
the other hand, to avoid annoying signature popups in the user's wallet for
calls that do not need to be signed, set the `from` address to all zeros. The
JS library will do this for you.

![Confidential Smart Contract Call Flow on Sapphire](../diagrams/c10l-smart-contract-call.mmd.svg)
