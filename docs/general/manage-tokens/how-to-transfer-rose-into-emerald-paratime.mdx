...
[Emerald](../developer-resources/emerald-paratime) is an EVM-compatible blockchain running inside the Oasis ParaTime. Because the balance of your ROSE wallet is stored on the consensus layer - outside of ParaTime's reach - there are special mechanisms for Emerald to access your tokens (formally defined in [ADR 0003](../../../oasis-core/adr/0003-consensus-runtime-token-transfer)).
...
Firstly, install the Oasis Wallet Browser extension, restore your existing or create a new Oasis wallet as described in the [Oasis Wallet Browser Extension chapter](oasis-wallets/browser-extension#create-a-new-wallet).
...
Advanced users (e.g. those running Emerald ParaTime [compute nodes](../run-a-node/set-up-your-node/run-a-paratime-node)) may need to withdraw ROSE stored in their `oasis1` Emerald account. Oasis Wallet Browser Extension does not support such withdrawals. Read the [Oasis CLI Deposit/Withdraw Tokens to/from ParaTime chapter](advanced/oasis-cli-tools/deposit-withdraw-tokens-to-from-paratime) to learn how to use Oasis CLI instead.
