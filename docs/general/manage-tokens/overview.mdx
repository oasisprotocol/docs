import DocCardList from '@theme/DocCardList';
import {findSidebarItem} from '@site/src/sidebarUtils';

# Overview

This documentation will guide you on how to use your ROSE tokens, where to keep them, how to transfer them, how to stake/delegate them, and more.

## Summary

* For **self-custody**, we recommend using one of our **official** [**Oasis Wallets**](oasis-wallets/), [Web](oasis-wallets/web) or [Browser Extension](oasis-wallets/browser-extension).
* ROSE is supported via three [**custody providers**](holding-rose-tokens/custody-providers): [Copper.co](https://copper.co), [Anchorage](https://anchorage.com) and [Finoa](https://finoa.io).
* If you want a self-custody **mobile app**, we recommend [Bitpie Wallet](holding-rose-tokens/bitpie-wallet).
* For extra security with self-custody, we recommend using the [Ledger](https://www.ledger.com) wallet with one of our official [Oasis Wallets](oasis-wallets/).
* For experienced developers/power users, we offer [Oasis CLI Tools](advanced/oasis-cli-tools/).

:::caution

For your own security and peace of mind, please only use wallets that are listed in our official documentation. Any other wallets are likely unofficial and may be subject to critical security vulnerabilities and other technical issues. Using wallets that not listed in our official documentation could result in the permanent loss of your ROSE tokens.

:::

## Quick Navigation

<DocCardList items={[
    findSidebarItem('manage-tokens/terminology'),
    findSidebarItem('manage-tokens/staking-and-delegating'),
    findSidebarItem('/general/manage-tokens/oasis-wallets'),
    findSidebarItem('/general/manage-tokens/advanced/oasis-cli-tools'),
    findSidebarItem('manage-tokens/holding-rose-tokens/custody-providers'),
    findSidebarItem('manage-tokens/holding-rose-tokens/bitpie-wallet'),
    findSidebarItem('manage-tokens/holding-rose-tokens/ledger-wallet'),
]} />
