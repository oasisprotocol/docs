...
To run a **ParaTime node** make sure to first set up a Validator node. Then set 
up a [Trusted Execution Environment (TEE)]. After that proceed to the [Run a 
ParaTime node].

If you are building a service on top of the Oasis Network, we suggest
that you set up a [non-validator node] and (optionally) a [ParaTime client
node], so your service do not depend on third party endpoints which are
often behind a traffic limiter.

[Hardware]: ./prerequisites/hardware-recommendations.md
[System]: ./prerequisites/system-configuration.mdx
[Oasis Node]: ./prerequisites/oasis-node.md
[Run a Validator Node]: ./set-up-your-node/run-validator.md
[Trusted Execution Environment (TEE)]: ./prerequisites/set-up-trusted-execution-environment-tee.md
