...
These instructions are for setting up a _ParaTime client_ node which only observes ParaTime activity and can submit transactions. If you want to run a _ParaTime_ node instead, see the [instructions for running a ParaTime node](run-a-paratime-node.mdx). Similarly, if you want to run a _validator_ or a _non-validator_ node instead, see the [instructions for running a validator node](run-validator.md) or [instructions for running a non-validator node](run-non-validator.md).
...
Reading the rest of the [ParaTime node setup instructions](run-a-paratime-node.mdx) may also be useful.
...
* `{{ runtime_id }}`: The [ParaTime identifier](run-a-paratime-node.mdx#the-paratime-identifier-and-binary).
  * You can find the current Oasis-supported ParaTime identifiers in the [Network Paramers](../../foundation/testnet/README.md#paratimes).
* `{{ non-sgx_runtime_bin_path }}`: Path to the [ParaTime's non-SGX binary](run-a-paratime-node.mdx#the-paratime-identifier-and-binary) of the form `/node/runtimes/foo-paratime`.
