...
These instructions are for setting up a _ParaTime client_ node which only observes ParaTime activity and can submit transactions. If you want to run a _ParaTime_ node instead, see the [instructions for running a ParaTime node](run-a-paratime-node). Similarly, if you want to run a _validator_ or a _non-validator_ node instead, see the [instructions for running a validator node](run-validator) or [instructions for running a non-validator node](run-non-validator).
...
Reading the rest of the [ParaTime node setup instructions](run-a-paratime-node) may also be useful.
...
* **The ParaTime Binary** contains the executable code that implements the ParaTime itself. It is executed in a sandboxed environment by Oasis Node and its format depends on whether the ParaTime is running in a Trusted Execution Environment (TEE) or not.
  ...
  For ParaTime client nodes, one _does not_ need to run the SGXS binary, even for ParaTimes running in a TEE. ParaTime client node always runs the regular Linux executable (an [ELF binary](https://en.wikipedia.org/wiki/Executable_and_Linkable_Format), usually without an extension).
...
* `{{ non-sgx_runtime_bin_path }}`: Path to the [ParaTime's non-SGX binary](run-a-paratime-node#the-paratime-identifier-and-binary) of the form `/node/runtimes/foo-paratime`.
