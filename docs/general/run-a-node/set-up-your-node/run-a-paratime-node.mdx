...
If you are looking for some concrete ParaTimes that you can run, see [the list of ParaTimes and their parameters](../../contribute-to-the-network/run-a-paratime-node).
...
Before following this guide, make sure you've followed the [Prerequisites](../prerequisites/) and [Run a Non-validator Node](run-non-validator) sections and have:
...
Reading the rest of the [validator node setup instructions](run-validator) may also be useful.

:::

:::info

To speed up bootstraping your new node, we recommend [copying node's state from your existing node](../advanced/copy-state-from-one-node-to-the-other) or [syncing it using state sync](../advanced/sync-node-using-state-sync).
...
To see the staking requirements for different node roles, use the Oasis CLI tools as described in [Common Staking Info](../../manage-tokens/advanced/oasis-cli-tools/common-staking-info).
...
To stake the tokens, use our [Oasis CLI tools](../../manage-tokens/advanced/oasis-cli-tools/delegate-tokens).

If everything was set up correctly, you should see something like below when running [Oasis Node Stake Account Info](../../manage-tokens/advanced/oasis-cli-tools/get-account-info) command for your entity's account (this is an example for the Testnet):
...
If you don't have an entity yet, create a new one by following the [Creating Your Entity](run-validator#creating-your-entity) instructions.
...
If you will be running the ParaTime on a new Oasis Node, initialize a new node by following the [Initializing a Node](run-validator#initializing-a-node) instructions.
...
Then generate and submit the new/updated entity descriptor via the entity registration transaction by following the [Generating Entity Registration Transaction](run-validator#generating-entity-registration-transaction) instructions.

:::caution

Make sure your entity descriptor (i.e. `entity.json`) is copied to your online server and saved as `/node/entity/entity.json` to ensure the [node's configuration](run-a-paratime-node#configuration) will find it.

:::

:::tip

You will [configure the node](run-a-paratime-node#configuration) to automatically register for the roles it has enabled (i.e. storage and compute roles) via the `worker.registration.entity` configuration flag.
...
* [**The ParaTime Identifier**](/oasis-core/runtime/identifiers) is a 256-bit unique identifier of a ParaTime on the Oasis Network. It provides a unique identity to the ParaTime and together with the [genesis document's hash](/oasis-core/consensus/genesis#genesis-documents-hash) serves as a domain separation context for ParaTime transaction and cryptographic commitments.
...
In case you decide to build the ParaTime binary from source yourself, make sure that you follow our [guidelines for deterministic compilation](/oasis-sdk/runtime/reproducibility) to ensure that you receive the exact same binary.
...
For ParaTimes running inside [Intel SGX Trusted Execution Environment](run-a-paratime-node#setting-up-trusted-execution-environment-tee), you will need to install the Oasis Core Runtime Loader.

The Oasis Core Runtime Loader binary (`oasis-core-runtime-loader`) is part of Oasis Core binary releases, so make sure you download the appropriate version specified the [Network Parameters](../../oasis-network/network-parameters) page.
...
If a ParaTime requires the use of a TEE, then make sure you set up TEE as instructed in the [Set up Trusted Execution Environment (TEE)](../prerequisites/set-up-trusted-execution-environment-tee) doc.
...
* `{{ runtime_id }}`: The [ParaTime identifier](run-a-paratime-node#the-paratime-identifier-and-binary).
  * You can find the current Oasis-supported ParaTime identifiers in the [Network Paramers](../../foundation/testnet/#paratimes).
* `{{ runtime_bin_path }}`: Path to the [ParaTime binary](run-a-paratime-node#the-paratime-identifier-and-binary) of the form `/node/runtimes/foo-paratime.sgxs`.
* `{{ runtime_sig_path }}`: Path to the [ParaTime detached signature](run-a-paratime-node#the-paratime-identifier-and-binary) (for ParaTimes that require Intel SGX) of the form `/node/runtimes/foo-paratime.sig`.
* `{{ ias_proxy_address }}`: The IAS proxy address in the form `ID@HOST:port`.
  * You can find the current Oasis IAS proxy address in the [Network Parameters](../../oasis-network/network-parameters).
  * If you want, you can also [run your own IAS proxy](run-an-ias-proxy).
...
See the general [Node troubleshooting](../troubleshooting) and [Set up TEE troubleshooting](../prerequisites/set-up-trusted-execution-environment-tee#troubleshooting) sections before proceeding with ParaTime node-specific troubleshooting.

### Too Old Bubblewrap Version

Double check your installed `bubblewrap` version, and ensure is at least of version **0.3.3**. Previous versions of this guide did not explicitly mention the required version. For details see the [Install Bubblewrap Sandbox](run-a-paratime-node#install-bubblewrap-sandbox-at-least-version-0-3-3) section.

### Stake Requirement

Double check your node entity satisfies the staking requirements for a ParaTime node. For details see the [Stake Requirements](run-a-paratime-node#stake-requirements) section.
