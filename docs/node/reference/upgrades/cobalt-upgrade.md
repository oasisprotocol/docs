...
For the actual steps that node operators need to make on their nodes, see the [Upgrade Log](../upgrade-logs/upgrade-log-mainnet.md#cobalt-upgrade).
...
Additionally, each runtime's state root will need to be updated for the [runtime storage migration](../upgrade-logs/upgrade-log-mainnet.md#runtime-operators) that is performed during this upgrade.
