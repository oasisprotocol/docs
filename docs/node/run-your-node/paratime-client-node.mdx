...
For example, running a ParaTime client node for an SGX-enabled ParaTime like Sapphire doesn't require having SGX on the ParaTime client node.
...
* `{{ runtime_orc_path }}`: Path to the [ParaTime bundle](paratime-client-node.mdx#manual-bundle-installation) of the form
...
If you want to download bundles from a registry other than the
[Oasis Bundle Registry], add the URL of your desired repositories to the runtime
configuration.
...
For more information, see [ADR25].

[Oasis Bundle Registry]: https://github.com/oasisprotocol/bundle-registry
[ADR25]: ../../adrs/0025-bundle_hot_loading/

### Manual Bundle Installation

In order to run an Emerald or custom ParaTime node you need to obtain an active
ParaTime bundle, see the Network Parameters page ([Mainnet], [Testnet]). The
bundle (`.orc` extension that stands for Oasis Runtime Container) contains all
the needed ParaTime binaries together with the identifier and version metadata
to ease deployment.
...
:::caution Compiling the ParaTime Binary from Source Code
...
For example, for the [Emerald ParaTime](../mainnet/README.md#emerald),
you would have to obtain the `emerald-paratime.orc` bundle and install it to
`/node/runtimes/emerald-paratime.orc`.
