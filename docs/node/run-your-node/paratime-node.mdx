...
runtime:
    paths:
        # Path to ParaTime bundles is required only for Emerald ParaTime.
        - {{ runtime_orc_path }}
...
* `{{ runtime_orc_path }}`: Path to the [ParaTime bundle](paratime-node.mdx#the-paratime-bundle) of the form
...
ROFL bundles must be configured statically. For detailed instructions, refer to
the [ROFL node].
...
In order to run an Emerald ParaTime node you need to obtain the ParaTime bundle
that needs to come from a trusted source. The bundle (usually with an `.orc`
