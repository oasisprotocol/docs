...
If you are looking for some concrete ParaTimes that you can run, see [the list of ParaTimes and their parameters](/general/contribute-to-the-network/run-a-paratime-node).
...
[Run a ParaTime Node]: /general/contribute-to-the-network/run-a-paratime-node
[Common Staking Info]: /general/manage-tokens/advanced/oasis-cli-tools/common-staking-info
[Oasis CLI tools]: /general/manage-tokens/advanced/oasis-cli-tools/delegate-tokens
[Oasis Node Stake Account Info]: /general/manage-tokens/advanced/oasis-cli-tools/get-account-info
...
that you follow our [guidelines for deterministic compilation](/developers/sdk/runtime/reproducibility)
...
The Oasis Core Runtime Loader binary (`oasis-core-runtime-loader`) is part of Oasis Core binary releases, so make sure you download the appropriate version specified the [Network Parameters](/general/oasis-network/network-parameters) page.
...
For example, for the [Cipher ParaTime](/general/oasis-network/network-parameters#cipher-paratime),
...
* `{{ runtime_orc_path }}`: Path to the [ParaTime bundle](run-a-paratime-node.mdx#the-paratime-bundle) of the form `/node/runtimes/foo-paratime.orc`.
  * You can find the current Oasis-supported ParaTimes in the [Network Paramers](/general/oasis-network/network-parameters#paratimes).
* `{{ ias_proxy_address }}`: The IAS proxy address in the form `ID@HOST:port`.
  * You can find the current Oasis IAS proxy address in the [Network Parameters](/general/oasis-network/network-parameters).
