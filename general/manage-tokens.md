...
[Oasis Explorer](https://explorer.oasis.io/mainnet/consensus) block explorer.
Enter your `oasis1` address in the search field at the top and hit enter.

The total balance is a sum of three values:

* the available tokens that can immediately be transferred,
* the staked (escrowed) tokens that are delegated,
* the debonding tokens that are waiting for the debonding period to pass.

The account's transactions include deposits, withdrawals and transfers.
You can view the details of a specific transaction by clicking on its hash.
Among others, you will see the transaction type, the "from", "to" and "amount"
fields.

**Info**:

If you want to explore Sapphire (`0x` addresses, token transfers, contract
calls, etc.), use the
