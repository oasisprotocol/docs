...
For the actual steps that node operators need to make on their nodes, see the [Upgrade Log](https://docs.oasis.io/node/reference/upgrade-logs/mainnet.md#cobalt-upgrade).
...
For a more detailed explanation of the parameters below, see the [Genesis Document](https://docs.oasis.io/node/reference/genesis-doc.md#parameters) docs.
...
The \*\*`epochtime`\*\*object will be removed since it became obsolete with the new [improved random beacon](https://docs.oasis.io/adrs/0007-improved-random-beacon.md). It will be replaced with the new **`beacon`** object described [below](https://docs.oasis.io/node/reference/upgrades/cobalt-upgrade.md#random-beacon).

### **Registry**

* \*\*`registry.params.enable_runtime_governance_models` \*\* is a new parameter that specifies the set of [runtime governance models](https://docs.oasis.io/core/consensus/services/registry.md#runtimes) that are allowed to be used when creating/updating registrations. It will be set to:
...
Deregistered entities can always re-register by submitting the [entity registration transaction](https://docs.oasis.io/node/run-your-node/validator-node.md#entity-registration) after the upgrade.
...
* \*\*`roothash.params.max_runtime_messages` \*\* is a new parameter that specifies the global limit on the number of [messages](https://docs.oasis.io/core/runtime/messages.md) that can be emitted in each round by the runtime. It will be set to `256`.
* **`roothash.params.max_evidence_age`** is a new parameter that specifies the maximum age (in the number of rounds) of submitted evidence for [compute node slashing](https://docs.oasis.io/adrs/0005-runtime-compute-slashing.md). It will be set to `100`.
...
* **`staking.params.allow_escrow_messages`** is a new parameter indicating whether to enable support for the newly added `AddEscrow` and `ReclaimEscrow` [runtime messages](https://docs.oasis.io/core/runtime/messages.md) . It will be set to`true`.
...
The **`beacon`** object contains parameters controlling the new [improved random beacon](https://docs.oasis.io/adrs/0007-improved-random-beacon.md) introduced in the Cobalt upgrade.

* **`beacon.base`** is the network's starting epoch. It will be set to the epoch of Mainnet's state dump + 1, i.e. `5047`.
* **`beacon.params.backend`** configures the random beacon backend to use. It will be set to `"pvss"` indicating that the beacon implementing a [PVSS (publicly verifiable secret sharing) scheme](https://docs.oasis.io/adrs/0007-improved-random-beacon.md) should be used.
...
The **`governance`** object contains parameters controlling the network's [on-chain governance](https://docs.oasis.io/core/consensus/services/governance.md) introduced in the Cobalt upgrade\*\*.\*\*
...
Additionally, each runtime's state root will need to be updated for the [runtime storage migration](https://docs.oasis.io/node/reference/upgrade-logs/mainnet.md#runtime-operators) that is performed during this upgrade.
...
For any additional support, please reach out via the [**#nodeoperators** Oasis Community Slack channel](https://docs.oasis.io/get-involved.md) with your questions, comments, and feedback related to Cobalt upgrade.
